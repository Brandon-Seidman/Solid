...
router.post("/complete", async (req, res) => {
  ...
  if (!user) {
    error(
      res,
      403,
      "You must be logged in as a valid user to complete a solid"
    );
    return;
  }
  ...
  if (user._id !== solid.buddyID) {
    error(
      res,
      403,
      "You must be the user that accepted this solid to complete it"
    );
    return;
  }
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    for (comment of solid.comments) {
      ...
      const commentPoster = await users.getUserByUsername(
        commentObject.commentBy
      );
